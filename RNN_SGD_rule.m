...
SGD_table.normalized_same_score = SGD_table.pro.*SGD_table.normalized_score +...
    (1 - SGD_table.pro).*-SGD_table.normalized_score;
SGD_table.time_step = num2str(SGD_table.time_step);
SGD_table.RNNid = num2str(SGD_table.RNNid);
%%
TABLE = SGD_table;
model = fitlme(TABLE, 'normalized_hit_rate ~ normalized_same_score * pro * switches * time_step + (normalized_same_score * pro * switches|RNNid) + (normalized_same_score * pro * switches|time_step)');
%model = fitlme(TABLE, 'normalized_hit_rate ~ normalized_same_score * pro * switches * time_step  + (normalized_same_score * pro * switches|time_step)');
%model = fitlme(TABLE, 'normalized_hit_rate ~ normalized_same_score * pro * switches');
...
save('mats/SGD_table_RNN_fitted.mat','SGD_matrix')

%% Compare y and y_hat vs x without pro/anti normalized
...
%% Compare y and y_hat vs x considering pro/anti normalized
this_condition = TABLE.time_step~='0' & TABLE.pro==1;
[binc,mu,se,n]=binned(TABLE.normalized_same_score(this_condition),...
...
this_condition = TABLE.time_step~='0' & TABLE.pro==0;
[binc,mu,se,n]=binned(TABLE.normalized_same_score(this_condition),...
    TABLE.normalized_hit_rate(this_condition),'n_bins',35);
[binc_fit,mu_fit,se_fit,n_fit]=binned(TABLE.normalized_same_score(this_condition),...
    TABLE.fit(this_condition),'n_bins',100);
...
clf;
to_plot = str2num(TABLE.RNNid)==11;
to_plot_pro = to_plot & TABLE.pro==1 & TABLE.time_step~='0';
to_plot_anti = to_plot & TABLE.pro==0 & TABLE.time_step~='0';
hold on
scatter(TABLE.normalized_same_score(to_plot_pro),TABLE.fit(to_plot_pro),'b')
scatter(TABLE.normalized_same_score(to_plot_anti),TABLE.fit(to_plot_anti),'r')