...
class VirtualRat(object):
    ...
    def _get_SGD_score(self, perceptrons, time_step, trial_type):
        ...
        TT = activation.shape[0]
        these_time_steps = np.arange(TT) % self.total_time_steps == time_step
        activation = activation[these_time_steps,0,:] # shape (T,H)
    ...
    def make_SGD_table(self, target=False, normalize = True):
        """
        Make an array (later converted to MATLAB table) to store SGDClassifier (linear regression classifier) information.
        """
        shift = 5
        categories = shift+self.total_time_steps*2 # RNN_index, pro, right, switch, hit_rate, (index 0~4)
                    # score0, score1, score2, score3, score4, (index 5~9)
                    # encoding0, encoding1, encoding2, encoding3, encoding4 (index 10~14)
        ...
        for trial_type in self.config_names:
            trial_num = score[0][trial_type]['score'].size
            ...
            if "switch" in trial_type:
                subtable[:,3] = 1
            subtable[:,4] = score[4][trial_type]['hit_rate']
            for t in range(self.total_time_steps):        
                SGD_score = score[t][trial_type]['score']
                subtable[:,shift+t] = SGD_score
            SGD_table = np.append(SGD_table,subtable,axis=0)
        # Normalize score
        if normalize:
            for t in range(self.total_time_steps):
                SGD_table[:,shift+t] = zscore(SGD_table[:,shift+t])
        # Compute encoding score:
        if target:
            right = SGD_table[:,2]
            for t in range(self.total_time_steps):
                SGD_score = SGD_table[:,shift+t]
                SGD_table[:,shift+self.total_time_steps+t] = right * SGD_score + (right-1) * SGD_score # Target encoding
        else:
            pro = SGD_table[:,1]
            for t in range(self.total_time_steps):
                SGD_score = SGD_table[:,shift+t]
                SGD_table[:,shift+self.total_time_steps+t] = pro * SGD_score + (pro-1) * SGD_score # Rule encoding
        return SGD_table