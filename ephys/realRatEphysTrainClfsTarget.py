...
for brain_area in brain_areas:
    ...
    accuracies, clfs = train_SGD(spike_count, goodSessionInfo,time_steps=time_steps, target = True)
    print "Making tables"
    normalized_table, session_table = make_tables(spike_count, goodSessionInfo,good_ratindex, clfs, 
        normalize = True, time_steps=time_steps, target = True)
    SGD_table_name = '../mats/'+experimentor+'SGD_table_target_'+"_".join(brain_area.split())+'.mat'
    session_table_name = '../mats/'+experimentor+'SGD_session_table_target_'+"_".join(brain_area.split())+'.mat'
