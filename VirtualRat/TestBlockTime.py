...
for j in range(start,end):
    ...
    try:
        loaded_params = load_weights("/scratch/xl1066/VirtualRat/poster/BlockTime/trainedBlockTime-"+str(block_length)+"-"+str(j)+"-"+str(0)+".pkl")
        print ratname
    except IOError:
        continue
    ...
    for i in range(num_loop+1):
        ...
        loaded_weights = load_weights("/scratch/xl1066/VirtualRat/poster/BlockTime/trainedBlockTime-"+str(block_length)+"-"+str(j)+"-"+str(i*epoch_per_loop)+".pkl")
        np.random.seed(j)
        npp.random.seed(j)
        solver.init()
...
save_weights("/gpfsnyu/home/xl1066/VirtualRat/poster/BlockTime/BlockTime-"+str(block_length)+"-"+str(end)+".pkl",matrices)