...
for j in [model_index]:
    ...
    save_weights("/scratch/xl1066/VirtualRat/poster/BlockTime/trainedBlockTime-"+str(block_length)+"-"+str(j)+'-'+str(0)+".pkl",params)
    for i in range(1,pre_train+1):
        solver.train()
        params = solver.save_params()
        save_weights("/scratch/xl1066/VirtualRat/poster/BlockTime/trainedBlockTime-"+str(block_length)+"-"+str(j)+'-'+str(i*epoch_per_loop)+".pkl",params)
        
    box.change_mode("alternative")
    for i in range(1,num_loop - pre_train+1):
        solver.train()
        params = solver.save_params()
        save_weights("/scratch/xl1066/VirtualRat/poster/BlockTime/trainedBlockTime-"+str(block_length)+"-"+str(j)+'-'+str((i+pre_train)*epoch_per_loop)+".pkl",params)
