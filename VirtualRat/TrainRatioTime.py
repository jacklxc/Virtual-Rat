...
num_loop = 100
pre_train = 30
p2a_ratio = (args.index/20)/10.0
for j in [args.index%20]:
    ...
    save_weights("/scratch/xl1066/VirtualRat/poster/RatioTime/trainedTrainingTimeRatio-"+str(p2a_ratio)+"-"+str(j)+'-'+str(0)+".pkl",params)
    for i in range(1,pre_train+1):
        solver.train()
        params = solver.save_params()
        save_weights("/scratch/xl1066/VirtualRat/poster/RatioTime/trainedTrainingTimeRatio-"+str(p2a_ratio)+"-"+str(j)+'-'+str(i*epoch_per_loop)+".pkl",params)
        
    box.change_mode("switch_ratio", p2a = p2a_ratio)
    for i in range(1,num_loop - pre_train+1):
        solver.train()
        params = solver.save_params()
        save_weights("/scratch/xl1066/VirtualRat/poster/RatioTime/trainedTrainingTimeRatio-"+str(p2a_ratio)+"-"+str(j)+'-'+str((i+pre_train)*epoch_per_loop)+".pkl",params)
