...
class SimpleRNN(object):
    ...
    def __init__(self, N=1, input_dim=3, output_dim=3, hidden_dim=5, 
        ...
        if params is None:
            ...
            # Initializa h0
            self.params['h0'] = np.random.randn(N, hidden_dim)
            self.params['h0'] /= np.sqrt(N)
            ...
            self.params['b'] = np.random.randn(hidden_dim)
            self.params['b'] /= np.sqrt(hidden_dim)
            # Initialize output to vocab weights
            self.params['Wa'] = np.random.randn(hidden_dim, output_dim)
            self.params['Wa'] /= np.sqrt(hidden_dim)
            self.params['ba'] = np.random.randn(output_dim)
            self.params['ba'] /= np.sqrt(output_dim)
    ...
    def loss(self, x, y):
        ...
        W_vocab, b_vocab = self.params['Wa'], self.params['ba']
        ...
        # Back prop
        dh, grads['Wa'],grads['ba'] = temporal_affine_backward(dscores,cache_scores)
    ...
    def predict(self, x):
        ...
        scores = np.zeros((N, T, DD))
        probability = np.zeros((N, T, DD))
