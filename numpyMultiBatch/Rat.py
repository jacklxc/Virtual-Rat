import numpy as np
class Rat(object):
    def __init__(self,ratname,rat, N, trial_window=3):
    ...
    def predict(self, RNN, final = False):
        ...
        if final:
            ...
            y = self.valY
            trueY = self.valTrueY
        choices, self.raw_scores, probs = self.RNN.predict(X)

        self.scores = self._concatBatches(self.raw_scores)
        X = self._concatBatches(X)
        trueY = self.compress_label(self._concatBatches(trueY))
        # Post RNN data process
        probs = self._softmax(self.raw_scores)
        self.probability = self._concatBatches(probs)
        ...
        return choices, self.probability
    ...
    def _softmax(self, scores):
        ...
        denomenator = np.sum(np.exp(scores))
