...
class Solver(object):
    ...
    def __init__(self, model, trainX, trainY, testX, testY, **kwargs):
        ...
        """
        self.model = model
        self.freeze_size = None
        self.freeze = False # Freeze loaded parameters or not
    ...
    def _reset(self):
        ...
        self.inputOutputHistory = np.zeros((0,6)) # Input dim 5 + action = 6
    ...
    def _step(self, X_batch, y_batch):
        ...
        for p, w in self.model.params.items():
            dw = grads[p]
            # Freeze the weights, i.e. kill the gradients
            if self.freeze:
                shape = self.freeze_size[p]
                if len(shape)==1:
                    dw[:shape[0]] = 0
                elif len(shape)==2:
                    dw[:shape[0],:shape[1]] = 0
    ...
    def check_accuracy(self, X, y):
        ...
        for i in xrange(X.shape[0]):
            X_batch = X[i]
            y_batch = y[i]
            predict = self.model.forward(X_batch, mode='test').asnumpy()
            # Customized!
            translator = np.array([0,1,2],dtype=np.int)
            # shape (N, batch_length)
            compressed_label = np.sum(y_batch * translator, axis=2)
            correct_or_not = np.argmax(predict, axis=2) == compressed_label
            if self.model.N == 1:
            	inputOutputHistory = np.append(X_batch[0,:,:],
            		np.expand_dims(correct_or_not.astype(int)[0,:],axis=1),axis=1)
            	self.inputOutputHistory = np.append(self.inputOutputHistory,inputOutputHistory,axis=0)
            acc_count += np.sum(correct_or_not)
    ...
    def load_params(self, params, freeze = False):
        """
        ...
        for k, v in params.iteritems():
            # Somehow simple way using full indices does not work.
            # So I have to use hard coded dimensions.
            self.freeze_size[k] = v.shape
            if len(v.shape)==1:
                self.model.params[k][:v.shape[0]] = v
            elif len(v.shape)==2:
                self.model.params[k][:v.shape[0],:v.shape[1]] = v
