...
class SimRat(object):
    ...
    def predict(self, X, y):
        ...
        self.RNN.reset_h()
        raw_scores = self.RNN.forward(X,"test").asnumpy()
        # Post RNN data process
        self.scores = self._concatBatches(raw_scores)
        X = self._concatBatches(self._new2oldX(self.X))
    ...
    def computeLearningCurve(self, inputOutputHistory, session_length = 150):
        """
        Takes inputOutputHistory and returns time series of pro accuracy and anti accuracy.
        """
        self.history = inputOutputHistory
