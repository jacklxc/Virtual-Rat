...
class SimRat(object):
    def __init__(self, RNN, ratname = "SimRat", trial_window=3):
        ...
        self.meanPerformanceExceptSwitchPro = None
        self.meanPerformanceExceptSwitchAnti = None
    def predict(self, X, y):
        ...
        self._meanPerformanceExceptSwitch()
        self.meanPerformanceExceptSwitchPro, self.meanPerformanceExceptSwitchAnti =  self._meanPerformanceExceptSwitch()
    ...
    def _meanPerformanceExceptSwitch(self):
        pro_prob = np.copy(self.pro_prob)
        anti_prob = np.copy(self.anti_prob)
        pro_prob[self.a2p] = np.nan
        anti_prob[self.p2a] = np.nan
        return np.nanmean(pro_prob), np.nanmean(anti_prob)